...
export default function P100ProSparePartsPage() {
  ...
  const fetchProducts = async () => {
    try {
      const res = await fetch('/api/admin/products?category=P100 Pro&limit=500');
    ...
    <div>
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
          ...
          <span>Repuestos</span>
          <span>/</span>
          <span className="text-gray-900">P100 Pro</span>
        </div>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">P100 Pro Spare Parts</h1>
            <p className="text-gray-500 mt-1">{products.length} products</p>
          </div>
          <div className="flex gap-3">
            <a
              href="/api/admin/products/export?category=P100 Pro"
            ...
            <Link
              href="/admin/products/new?category=P100 Pro"
        ...
        {filteredProducts.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {searchTerm ? 'Try a different search term.' : 'Add some P100 Pro spare parts to get started.'}
            </p>
          </div>
        )}
      </div>
